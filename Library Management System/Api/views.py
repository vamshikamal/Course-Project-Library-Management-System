...
def books():
    if "Logged_in" in session:
        ...
        books = bookListSchema.dump(books)
        print(books)
...
@app.route("/issuebook-ajax", methods=['GET', 'POST'])
def issue_Book():
    if "Logged_in" in session:
        isbn = request.json['isbn']    
        ...
        if book != None :
            ...
            if book.Book_copies == 0:
                return "NAV"
            book = Book_Borrowed(bookID = isbn, userID = session['id'], Issue_date = date)
            db.session.add(book)
            db.session.commit()
            book_item = Book.query.filter_by(ISBN=isbn).first()
            book_item.Book_copies = book_item.Book_copies - 1
            db.session.commit()
            return 'success'
        else:
            return "Sorry"
...
@app.route("/user_issued_books", methods=['GET', 'POST'])
def user_issued_books():
    if "Logged_in" in session:
        books = Book_Borrowed.query.filter(Book_Borrowed.userID==session["id"]).all()
        books = book_borrowed_ListSchema.dump(books)
        print(books)
        return jsonify(books)
    else:
        return "logged_out"
